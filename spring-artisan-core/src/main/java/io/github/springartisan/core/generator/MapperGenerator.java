...
public class MapperGenerator extends BaseGenerator {
    public MapperGenerator(GeneratorConfig config, TemplateEngine templateEngine) {
        super(config, templateEngine);
    }

    @Override
    public String getTemplateName() {
        return "mapper" + templateSuffix();
    }

    @Override
    public String getOutputPath(EntityDefinition entity) {
        String packagePath = packageToPath(config.getModelPackage().replace(".model", ".mapper"));
        return packagePath + "/" + entity.getName() + "Mapper" + fileExtension();
    }
}
